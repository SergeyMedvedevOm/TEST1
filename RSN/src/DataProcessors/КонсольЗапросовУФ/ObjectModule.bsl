...
//
// ВнешниеОбработки.Создать("C:\КонсольЗапросовУФ.epf", Ложь).ОбработатьВнешнийЗапрос(Запрос, "C:\РезультатыЗапроса.txt");
//
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Процедура ОбработатьЗапрос(ппСтруктураДанных = Неопределено, ппМВТ = Неопределено) Экспорт
	...
	// -- 08.09.2015 Доработка от tristarr1
		...
		Для каждого ЭлементСтруктуры Из ппХранилище.СтруктураТЗ Цикл
			Если ВхождениеСтроки(ЭлементСтруктуры.Ключ, СтрокаПоиска) Тогда
		...
		Для каждого СтрокаПакета Из ТаблицаПакетов Цикл
			...
			Если пПоказать Тогда
				...
				Для СчКол = 1 По КоличествоКолонок Цикл
					...
					//	
					//	Если ТипВсеСсылки_Документы.СодержитТип(ТипЗнач) Тогда
					//		
					...
					//		
					//	ИначеЕсли ТипВсеСсылки_Перечисления.СодержитТип(ТипЗнач)Тогда
					//		
					//		ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗнач);
					//		
					//		Ширина = 0;
					...
					ШиринаКолонки = ШиринаКолонки * КоффициентШириныКолонок(ШиринаКолонки);
...
Функция СоздатьТаблицуВТ(Знач ТекстДляРазбора) Экспорт
	...
	Пока СтрДлина(ТекстДляРазбора) > 0 Цикл
		...
		НачалоВЫБРАТЬ = ВхождениеСтроки(ВРег(ТекстДляРазбора), "ВЫБРАТЬ");
		Если НачалоВЫБРАТЬ = 0 Тогда
			НачалоВЫБРАТЬ = ВхождениеСтроки(ВРег(ТекстДляРазбора), "SELECT");
		КонецЕсли; 
		
		НачалоУНИЧТОЖИТЬ = ВхождениеСтроки(ВРег(ТекстДляРазбора), "УНИЧТОЖИТЬ");
		Если НачалоУНИЧТОЖИТЬ = 0 Тогда
			НачалоУНИЧТОЖИТЬ = ВхождениеСтроки(ВРег(ТекстДляРазбора), "DROP");
		КонецЕсли; 
		...
		КонецПакета = ВхождениеСтроки(ТекстДляРазбора, ";");
		...
		ПрисутствиеВТ = ВхождениеСтроки(ВРег(ТекстПакета), "ПОМЕСТИТЬ");
		Если ПрисутствиеВТ > 0 Тогда
		    Оператор_Поместить_INTO = "ПОМЕСТИТЬ";
		КонецЕсли; 
		
		Если ПрисутствиеВТ = 0 Тогда
			ПрисутствиеВТ = ВхождениеСтроки(ВРег(ТекстПакета), "INTO");
			Если ПрисутствиеВТ > 0 Тогда
...
Функция ОбработатьВнешнийЗапрос(Знач Запрос, ПутьКФайлуРезультата) Экспорт
	...
	//ПутьКФайлуРезультата = ФайлОбработки.Путь + ИмяФайла;
	ФайлРезультата = Новый Файл(ПутьКФайлуРезультата);
	//ФайлРезультата.Расширение = ".txt";
	...
	Для i = 1 По КоличествоСтрок Цикл
		...
		Для r = 1 По СимволовВСтроке Цикл
			...
			ИначеЕсли ЭтоТекст ИЛИ (НЕ ЭтоТекст И ТекСимвол = """") Тогда
				НоваяСтрока = НоваяСтрока + ТекСимвол;
			ИначеЕсли ВхождениеСтроки(СимволыРазделители, ТекСимвол) > 0 Тогда
				НоваяСтрока = НоваяСтрока + ТекСимвол;
				Если ТекСимвол = "(" Тогда
			...
			Иначе
				...
				Пока r < СимволовВСтроке Цикл
				    r = r + 1;
					ТекСимвол = Сред(ТекущаяСтрока, r, 1);
					Если ВхождениеСтроки(СимволыРазделители, ТекСимвол) = 0 Тогда
...
Функция ПроверитНаличиеИспользуемыхПеременных(Знач Текст)
	...
	Для каждого Элемент Из МассивСлов Цикл
		...
		Если ВхождениеСтроки(Текст2, ВРег(" " + Элемент + " ")) Тогда
...
Функция КоффициентШириныКолонок(Ширина)
...
КонецФункции // КоффициентШириныКолонок()
