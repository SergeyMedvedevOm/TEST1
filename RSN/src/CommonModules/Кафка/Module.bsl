...
Функция НовыйПодписчик( ОписаниеСоединения, 
						...
						МаксимальныйРазмерОтветаБайт = 1000) Экспорт
...
Функция ПолучитьСообщения(Подписчик) Экспорт 
	...
	//++ иначе заголовки затираются значениями по умолчанию.
	Заголовки.Вставить("НеОбъединятьЗаголовки", Истина);
	//--
...
Процедура ПодтвердитьПолучение(Подписчик, СвойстваПодтверждения) Экспорт 
    ...
    Оффсет.Вставить("topic", СвойстваПодтверждения.topic);
    Оффсет.Вставить("partition", СвойстваПодтверждения.partition);
    Оффсет.Вставить("offset", СвойстваПодтверждения.offset);
